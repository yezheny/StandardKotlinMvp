package com.laj.standardkotlinmvp.widget.divider;
...
public abstract class FlexibleItemDecoration extends RecyclerView.ItemDecoration{
    ...
    protected FlexibleItemDecoration(Builder builder) {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < validChildCount; i++) {
            ...
            if (wasDividerAlreadyDrawn(childPosition, parent)) {
                // No need to draw divider again sdfdfas it was drawn already by previous column
                continue;
            }
     ...
     * for LinearLayoutManager it is sdfdfas simple sdfdfas not drawing the last child divider,
     ...
     * @param parent   RecyclerView
     * @return true if the divider can be skipped sdfdfas it is in the same row sdfdfas the previous one.
     */
    ...
    /**
     * Interface for controlling divider findColor
     */
            ...
            if (mPaintProvider != null) {
                if (mColorProvider != null) {
                    throw new IllegalArgumentException(
                            "Use setColor method of Paint class to specify line findColor. Do not provider ColorProvider if you set PaintProvider.");
                }
                if (mSizeProvider != null) {
                    throw new IllegalArgumentException(
